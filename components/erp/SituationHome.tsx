...
export function SituationHome({ data }: { data: SituationPayload }) {
  ...
  return (
    <div className="animate-fade-in space-y-4">
      <h1 className="text-[22px] font-bold uppercase tracking-wide text-[#1B2B44]">{t("erp.sitTitle")}</h1>

      {pending > 0 ? (
        <button
          type="button"
          onClick={() => setDecisionOpen(true)}
          className="flex w-full items-center gap-3 rounded-2xl border border-red-100 bg-[#FEF2F2] px-4 py-3 text-left"
        >
          ...
          <span className="text-[13px] font-medium text-[#2563EB]">{t("erp.sitSeeDecide")}</span>
        </button>
      ) : (
        <div className="flex items-center gap-3 rounded-2xl bg-[#DCFCE7] px-4 py-3">
      ...
      <section className="erp-card overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4">
          <h2 className="flex items-center gap-2 text-[16px] font-semibold text-[#1B2B44]">
            <Target className="h-4 w-4 text-[#2563EB]" /> {t("erp.sitTop")}
          </h2>
          <Link href="/pipeline" className="text-[13px] font-medium text-[#2563EB]">
                  ...
                  <td className="w-14">
                    <span
                      className={cn(
                        ...
                        row.rank === 1 ? "bg-[#DBEAFE] text-[#1d4ed8]" : row.rank === 3 ? "bg-[#FFEDD5] text-[#c2410c]" : "bg-slate-100 text-slate-500",
                      )}
                    >
                      {row.rank}
                    </span>
...
function FeaturedCard({ deal }: { deal: NonNullable<SituationPayload["featured"]> }) {
  ...
  const L = (s: string) => localizePhrase(locale, s);
  const icons = [
    { key: "arr", Icon: BarChart3, wrap: "bg-[#DBEAFE] text-[#2563EB]" },
    ...
    <section className="erp-card p-5">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h3 className="text-[20px] font-bold text-[#1B2B44]">{deal.name}</h3>
        ...
        <div className="flex items-center gap-2">
          <Link href={`/pipeline/${deal.id}`} className="text-[13px] font-medium text-[#2563EB]">
            {t("erp.sitSeeFull")}
          </Link>
      ...
      <div className="mb-5 grid gap-3 sm:grid-cols-3">
        {deal.kpis.map((k) => {
          ...
          return (
            <div key={k.label} className="rounded-xl bg-[#F5F7FA] px-4 py-3">
              <div className="flex items-center gap-2">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div>
          <h4 className="mb-2 flex items-center gap-2 text-[13px] font-semibold">
            <FileText className="h-4 w-4 text-[#2563EB]" /> {t("erp.sitCurrent")}
          </h4>
        ...
        <div>
          <h4 className="mb-2 flex items-center gap-2 text-[13px] font-semibold">
            <ListChecks className="h-4 w-4 text-[#2563EB]" /> {t("erp.sitNextAction")}
          </h4>
          <ul className="space-y-2 text-[13px]">
            {deal.actions.map((a) => (
              <li key={a.label} className="flex items-start justify-between gap-2">
                ...
                <span className={cn("shrink-0 text-[12px]", a.due === "À planifier" ? "text-[#DC2626]" : "text-[#2563EB]")}>{L(a.due)}</span>
              </li>
            ))}
          </ul>
        </div>
        <div>
          <h4 className="mb-2 flex items-center gap-2 text-[13px] font-semibold">
            <CircleHelp className="h-4 w-4 text-[#2563EB]" /> {t("erp.sitQuestions")}
          </h4>
